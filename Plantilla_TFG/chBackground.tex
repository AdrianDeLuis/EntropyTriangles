...
\todo[inline]{Include a paragraph explaining what you are doing in the chaper. Applicable to all chapters. }
...
\todo[inline]{All the notation below relates to random vectors, not to random variables. Accordingly, all variables have to be vectors such as $\overline X$. }
...
\todo[inline]{In the following paragraph the indexing is of layers is not done apropriately. You need to use $h_i$ and $h_{i-1} $ throughout.}
...
\todo[inline]{FVA: open up an ``abbreviation section'' and include ITL as abbrev. }
...
\todo[inline]{FVA: explain also the meaning of the $\lambda$ parameter.}
\begin{figure}[H]
	\centering	

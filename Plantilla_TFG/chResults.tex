...
% which given that the observations from the variables have different ranges of values
...
%, which as mentioned before was a column with a constant value of 0 all throughout the set. 
...
Just by taking a simple look over Figure~\ref{fig:figure_Knn_Iono}.\subref{fig:figure_Knn_Iono_ET_Auto} and Figure~\ref{fig:figure_Knn_Iono}.\subref{fig:figure_Knn_Iono_ET_PCA} 
...
%
%The coordinates of the entropies from our KNN Folds tells us that our predictions have been inaccurate and the transmission of information either by means of the Virtual Information or the Mutual Information have degraded a lot from the results gathered from the Iris dataset.
% FVA: This is the correct analysis
%
\begin{figure}[H]
...
Since we have presented the results of the $5$-folds in the validation it is possible to obtain a representation of the dispersion of the measures. 
In fact Figures~\ref{fig:figure_Knn_Iono_Confidence}.\subref{fig:figure_Knn_Iono_ET_Auto_Confidence} and~\ref{fig:figure_Knn_Iono_Confidence}.\subref{fig:figure_Knn_Iono_ET_PCA_Confidence}, 
 with a reduced dispersion area, 
suggest that the results results mentioned above are quite consistent. 
%point to us that sparsity of possible new test values is not very big, which basically is indicating that our classifications can't greatly vary their inaccurate predictions.\par 
%
\begin{figure}[H]
\begin{subfigure}{\textwidth}  
	\centering
...
We can also notice in the figure how the dispersion of the training results is smaller than that of the testing results. 

\subsubsection{MLP on PCA and Autoencoder}

%To completely assess the Ionosphere dataset classification process we must now assess the performance of the MLP:
...
\subsection{The Autoencoder}
%The Autoencoder used for the MNIST dataset has the most complex structure in terms of computational power out of all of the considered implementations in this thesis. 
...
From Table \ref{tab:table_MNIST_MLP_pca} the PCA MLP has the advantage of being composed by a greater amount of neurons, making it more computationally powerful and thus having an advantage towards the smaller Autoencoder MLP. 
%
\begin{table}[H]
...
After plotting both Figure~\ref{fig:figure_Mlp_MNIST_ET_Auto}.\subref{fig:figure_Mlp_MNIST_ET_Auto} and Figure~\ref{fig:figure_Mlp_MNIST_ET_Auto}.\subref{fig:figure_Mlp_MNIST_ET_PCA} we can clearly see that the PCA  proves to be the best choice to classify MNIST with an MLP. Its training folds have placed in the tip of the ET, and although the test folds have performed worst than them, they are still placed highly. On the other hand, the MLP classifier on the Autoencoder has good informational characteristic, but it lacks the predictive power of the PCA.
%
\begin{figure}[H]
\begin{subfigure}{\textwidth}
	\centering
